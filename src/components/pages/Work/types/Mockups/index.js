...
function Mockups({ title, description, link, repo, react, desktopImg, mobileImg, flip }) {
	...
	return (
		<div className='sticky-container'>
			<div id={id} className={flip ? 'project-preview-right' : 'project-preview-left'}>
				<div className={flip ? 'preview-thumbnail-right' : 'preview-thumbnail-left'}>
					<a href={link} target='_blank' rel='noreferrer' className='project-thumbnail-desktop-link' style={{ transform: `translateY(-${Math.floor(modifier / 5)}px)` }}>
						<img src={desktopImg} alt='recipe generator desktop preview' className='project-thumbnail-desktop'></img>
					</a>
					...
					<a href={link} target='_blank' rel='noreferrer' className='project-thumbnail-phone-link' style={{ transform: `translateY(${Math.floor(modifier / 10)}px)` }}>
						<img src={mobileImg} alt='recipe generator phone preview' className='project-thumbnail-phone'></img>
					</a>
				</div>
